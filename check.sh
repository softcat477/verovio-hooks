...
format_diff() {
	...
	if [[ $CLANG_FORMAT_VERSION -gt "9" ]]; then
        # echo docker run -i -v "$(pwd)":"$(pwd)" -w "$(pwd)" --rm ghcr.io/jidicula/clang-format:"$CLANG_FORMAT_VERSION" -n --Werror --style=file --fallback-style="$FALLBACK_STYLE" "${filepath}"
		local_format="$(docker run -i -v "$(pwd)":"$(pwd)" -w "$(pwd)" --rm ghcr.io/jidicula/clang-format:"$CLANG_FORMAT_VERSION" -i --Werror --style=file --fallback-style="$FALLBACK_STYLE" "${filepath}")"
...
#src_files=$(find -E "$CHECK_PATH" -name .git -prune -o -regex '^.*\.((((c|C)(c|pp|xx|\+\+)?$)|((h|H)h?(pp|xx|\+\+)?$))|(ino|pde)|(proto))$' -print)

# CHECK_PATH is a path -> search files in the path that match regex
if [[ -d $CHECK_PATH ]]; then
	echo "Search files in $CHECK_PATH"
	src_files=$(find -E "$CHECK_PATH" -name .git -prune -o -regex '^.*\.((((c|C)(c|pp|xx|\+\+)?$)|((h|H)h?(pp|xx|\+\+)?$))|(ino|pde)|(proto))$' -print)
	#src_files=$(find "$CHECK_PATH" -name .git -prune -o -regextype posix-egrep -regex '^.*\.((((c|C)(c|pp|xx|\+\+)?$)|((h|H)h?(pp|xx|\+\+)?$))|(ino|pde)|(proto))$' -print)
else
	# CHECK_PATH are files (probably used by pre-commit)
	src_files=$CHECK_PATH
fi
...
for file in $src_files; do
	...
	if ! [[ ${file} =~ $EXCLUDE_REGEX ]]; then
		if [[ $file =~ ^.*\.((((c|C)(c|pp|xx|\+\+)?$)|((h|H)h?(pp|xx|\+\+)?$))|(ino|pde)|(proto))$ ]]; then
			echo "Reformat $file: clang-format -i --Werror --style=file --fallback-style=llvm $file"
			format_diff "${file}"
		fi
	fi
done
